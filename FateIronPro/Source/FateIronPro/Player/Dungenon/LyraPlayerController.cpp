...
#include "GameModes/Dungenon/FIDungeonGameState.h"
...
UFIAbilitySystemComponent* ALyraPlayerController::GetLyraAbilitySystemComponent() const
{
...
// 	if (CheatManager)
// 	{
// 		UE_LOG(LogFI, Warning, TEXT("ServerCheat: %s"), *Msg);
...
// 	if (CheatManager)
// 	{
// 		UE_LOG(LogFI, Warning, TEXT("ServerCheatAll: %s"), *Msg);
...
{
	// if (UFIAbilitySystemComponent* LyraASC = GetLyraAbilitySystemComponent())
	// {
...
bool ALyraPlayerController::GetIsAutoRunning() const
{
	...
	// if (const UFIAbilitySystemComponent* LyraASC = GetLyraAbilitySystemComponent())
	// {
...
// {
// 	UE_LOG(LogFITeams, Error, TEXT("You can't set the team ID on a player controller (%s); it's driven by the associated player state"), *GetPathNameSafe(this));
// }
//
...
void ALyraReplayPlayerController::Tick(float DeltaSeconds)
{
	...
	if (!IsValid(FollowedPlayerState))
	{
		...
		if (AFIDungeonGameState* GameState = Cast<AFIDungeonGameState>(World->GetGameState()))
		{
