...
// #include "GameModes/FIDungeonGameMode.h"
#include "LyraPlayerController.h"
#include "AbilitySystem/FIAbilitySet.h"
#include "AbilitySystem/FIAbilitySystemComponent.h"
#include "AbilitySystem/Attributes/FICombatSet.h"
#include "AbilitySystem/Attributes/FIHealthSet.h"
#include "Character/FIPawnData.h"
#include "GameModes/Dungenon/FIDungeonGameMode.h"
...
{
	if (AFIDungeonGameMode* LyraGameMode = GetWorld()->GetAuthGameMode<AFIDungeonGameMode>())
	{
		if (const UFIPawnData* NewPawnData = LyraGameMode->GetPawnDataForController(GetOwningController()))
		{
			SetPawnData(NewPawnData);
		}
		else
		{
			UE_LOG(LogFI, Error, TEXT("ALyraPlayerState::OnExperienceLoaded(): Unable to find PawnData to initialize player state [%s]!"), *GetNameSafe(this));
		}
	}
}
...
void ALyraPlayerState::SetPawnData(const UFIPawnData* InPawnData)
{
	// check(InPawnData);
	//
	...
	// if (PawnData)
	// {
	// 	UE_LOG(LogFI, Error, TEXT("Trying to set PawnData [%s] on player state [%s] that already has valid PawnData [%s]."), *GetNameSafe(InPawnData), *GetNameSafe(this), *GetNameSafe(PawnData));
	// 	return;
	// }
	//
	...
	// PawnData = InPawnData;
	//
	// for (const UFIAbilitySet* AbilitySet : PawnData->AbilitySets)
	// {
	// 	if (AbilitySet)
	// 	{
...
// 	else
// 	{
// 		UE_LOG(LogFITeams, Error, TEXT("Cannot set team for %s on non-authority"), *GetPathName(this));
// 	}
// }
//
