...
#include "FIHealthComponent.generated.h"
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_FourParams(FLyraHealth_AttributeChanged, UFIHealthComponent*, HealthComponent, float, OldValue, float, NewValue, AActor*, Instigator);
...
class FATEIRONPRO_API UFIHealthComponent : public UGameFrameworkComponent
{
	GENERATED_BODY()

public:

	UFIHealthComponent(const FObjectInitializer& ObjectInitializer);

	// Returns the health component if one exists on the specified actor.
	UFUNCTION(BlueprintPure, Category = "Lyra|Health")
	static UFIHealthComponent* FindHealthComponent(const AActor* Actor) { return (Actor ? Actor->FindComponentByClass<UFIHealthComponent>() : nullptr); }

	// Initialize the component using an ability system component.
	UFUNCTION(BlueprintCallable, Category = "Lyra|Health")
	void InitializeWithAbilitySystem(UFIAbilitySystemComponent* InASC);
...
protected:
	...
	UPROPERTY()
	TObjectPtr<const UFIHealthSet> HealthSet;
