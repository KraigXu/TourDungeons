 ...
 */
UCLASS()
class UFICameraComponent : public UCameraComponent
{
	GENERATED_BODY()

public:

	UFICameraComponent(const FObjectInitializer& ObjectInitializer);

	// Returns the camera component if one exists on the specified actor.
	UFUNCTION(BlueprintPure, Category = "Lyra|Camera")
	static UFICameraComponent* FindCameraComponent(const AActor* Actor) { return (Actor ? Actor->FindComponentByClass<UFICameraComponent>() : nullptr); }
