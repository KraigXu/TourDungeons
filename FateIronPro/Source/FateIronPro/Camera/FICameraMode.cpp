...
UFICameraComponent* UFICameraMode::GetLyraCameraComponent() const
{
	return CastChecked<UFICameraComponent>(GetOuter());
}

UWorld* UFICameraMode::GetWorld() const
{
	return HasAnyFlags(RF_ClassDefaultObject) ? nullptr : GetOuter()->GetWorld();
}

AActor* UFICameraMode::GetTargetActor() const
{
	const UFICameraComponent* LyraCameraComponent = GetLyraCameraComponent();

	return LyraCameraComponent->GetTargetActor();
}

FVector UFICameraMode::GetPivotLocation() const
{
...
FRotator UFICameraMode::GetPivotRotation() const
{
	const AActor* TargetActor = GetTargetActor();
	check(TargetActor);
...
void UFICameraMode::UpdateCameraMode(float DeltaTime)
{
	UpdateView(DeltaTime);
	UpdateBlending(DeltaTime);
}

void UFICameraMode::UpdateView(float DeltaTime)
{
...
void UFICameraMode::SetBlendWeight(float Weight)
{
...
void UFICameraMode::UpdateBlending(float DeltaTime)
{
	if (BlendTime > 0.0f)
	{
...
void UFICameraMode::DrawDebug(UCanvas* Canvas) const
{
	check(Canvas);
...
void ULyraCameraModeStack::ActivateStack()
{
	if (!bIsActive)
	{
		...
		for (UFICameraMode* CameraMode : CameraModeStack)
		{
			check(CameraMode);
...
void ULyraCameraModeStack::PushCameraMode(TSubclassOf<UFICameraMode> CameraModeClass)
{
	if (!CameraModeClass)
	{
		return;
	...
	UFICameraMode* CameraMode = GetCameraModeInstance(CameraModeClass);
	check(CameraMode);
...
UFICameraMode* ULyraCameraModeStack::GetCameraModeInstance(TSubclassOf<UFICameraMode> CameraModeClass)
{
	check(CameraModeClass);

	// First see if we already created one.
	for (UFICameraMode* CameraMode : CameraModeInstances)
	{
	...
	UFICameraMode* NewCameraMode = NewObject<UFICameraMode>(GetOuter(), CameraModeClass, NAME_None, RF_NoFlags);
	check(NewCameraMode);
...
void ULyraCameraModeStack::UpdateStack(float DeltaTime)
{
	...
	for (int32 StackIndex = 0; StackIndex < StackSize; ++StackIndex)
	{
		UFICameraMode* CameraMode = CameraModeStack[StackIndex];
		check(CameraMode);
...
void ULyraCameraModeStack::BlendStack(FLyraCameraModeView& OutCameraModeView) const
{
	...
	const UFICameraMode* CameraMode = CameraModeStack[StackSize - 1];
	check(CameraMode);
...
void ULyraCameraModeStack::DrawDebug(UCanvas* Canvas) const
{
	...
	for (const UFICameraMode* CameraMode : CameraModeStack)
	{
		check(CameraMode);
...
void ULyraCameraModeStack::GetBlendInfo(float& OutWeightOfTopLayer, FGameplayTag& OutTagOfTopLayer) const
{
	...
	{
		UFICameraMode* TopEntry = CameraModeStack.Last();
		check(TopEntry);
