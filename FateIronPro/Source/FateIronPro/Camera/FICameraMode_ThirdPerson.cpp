...
#include "FICameraAssistInterface.h"
...
void UFICameraMode_ThirdPerson::UpdatePreventPenetration(float DeltaTime)
{
	...
	IFICameraAssistInterface* TargetControllerAssist = Cast<IFICameraAssistInterface>(TargetController);

	IFICameraAssistInterface* TargetActorAssist = Cast<IFICameraAssistInterface>(TargetActor);
	...
	IFICameraAssistInterface* PPActorAssist = OptionalPPTarget.IsSet() ? Cast<IFICameraAssistInterface>(PPActor) : nullptr;
	...
	if (PPActorRootComponent)
	{
		...
		IFICameraAssistInterface* AssistArray[] = { TargetControllerAssist, TargetActorAssist, PPActorAssist };

		if (AimLineToDesiredPosBlockedPct < ReportPenetrationPercent)
		{
			for (IFICameraAssistInterface* Assist : AssistArray)
			{
				if (Assist)
				{
