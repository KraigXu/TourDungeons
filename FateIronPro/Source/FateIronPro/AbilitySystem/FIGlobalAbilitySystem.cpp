...
#include UE_INLINE_GENERATED_CPP_BY_NAME(FIGlobalAbilitySystem)

void FGlobalAppliedAbilityList::AddToASC(TSubclassOf<UGameplayAbility> Ability, UFIAbilitySystemComponent* ASC)
{
...
void FGlobalAppliedAbilityList::RemoveFromASC(UFIAbilitySystemComponent* ASC)
{
	if (FGameplayAbilitySpecHandle* SpecHandle = Handles.Find(ASC))
	{
...
void FGlobalAppliedEffectList::AddToASC(TSubclassOf<UGameplayEffect> Effect, UFIAbilitySystemComponent* ASC)
{
...
void FGlobalAppliedEffectList::RemoveFromASC(UFIAbilitySystemComponent* ASC)
{
	if (FActiveGameplayEffectHandle* EffectHandle = Handles.Find(ASC))
	{
...
UFIGlobalAbilitySystem::UFIGlobalAbilitySystem()
{
}

void UFIGlobalAbilitySystem::ApplyAbilityToAll(TSubclassOf<UGameplayAbility> Ability)
{
	if ((Ability.Get() != nullptr) && (!AppliedAbilities.Contains(Ability)))
	{
		FGlobalAppliedAbilityList& Entry = AppliedAbilities.Add(Ability);		
		for (UFIAbilitySystemComponent* ASC : RegisteredASCs)
		{
...
void UFIGlobalAbilitySystem::ApplyEffectToAll(TSubclassOf<UGameplayEffect> Effect)
{
...
void UFIGlobalAbilitySystem::RemoveAbilityFromAll(TSubclassOf<UGameplayAbility> Ability)
{
	if ((Ability.Get() != nullptr) && AppliedAbilities.Contains(Ability))
	{
...
void UFIGlobalAbilitySystem::RemoveEffectFromAll(TSubclassOf<UGameplayEffect> Effect)
{
	if ((Effect.Get() != nullptr) && AppliedEffects.Contains(Effect))
	{
...
void UFIGlobalAbilitySystem::RegisterASC(UFIAbilitySystemComponent* ASC)
{
	check(ASC);
...
void UFIGlobalAbilitySystem::UnregisterASC(UFIAbilitySystemComponent* ASC)
{
	check(ASC);
	for (auto& Entry : AppliedAbilities)
	{
