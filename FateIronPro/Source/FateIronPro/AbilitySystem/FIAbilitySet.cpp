...
#include "FIAbilitySet.h"

#include "FIAbilitySystemComponent.h"
#include "FateIronPro/FILogChannels.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(FIAbilitySet)
...
void FLyraAbilitySet_GrantedHandles::TakeFromAbilitySystem(UFIAbilitySystemComponent* LyraASC)
{
	check(LyraASC);
...
UFIAbilitySet::UFIAbilitySet(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

void UFIAbilitySet::GiveToAbilitySystem(UFIAbilitySystemComponent* LyraASC, FLyraAbilitySet_GrantedHandles* OutGrantedHandles, UObject* SourceObject) const
{
	check(LyraASC);
	...
	for (int32 AbilityIndex = 0; AbilityIndex < GrantedGameplayAbilities.Num(); ++AbilityIndex)
	{
		...
		{
			UE_LOG(LogFIAbilitySystem, Error, TEXT("GrantedGameplayAbilities[%d] on ability set [%s] is not valid."), AbilityIndex, *GetNameSafe(this));
			continue;
		}

		UFIGameplayAbility* AbilityCDO = AbilityToGrant.Ability->GetDefaultObject<UFIGameplayAbility>();
	...
	for (int32 EffectIndex = 0; EffectIndex < GrantedGameplayEffects.Num(); ++EffectIndex)
	{
		...
		{
			UE_LOG(LogFIAbilitySystem, Error, TEXT("GrantedGameplayEffects[%d] on ability set [%s] is not valid"), EffectIndex, *GetNameSafe(this));
			continue;
		}
	...
	for (int32 SetIndex = 0; SetIndex < GrantedAttributes.Num(); ++SetIndex)
	{
		...
		{
			UE_LOG(LogFIAbilitySystem, Error, TEXT("GrantedAttributes[%d] on ability set [%s] is not valid"), SetIndex, *GetNameSafe(this));
			continue;
		}
