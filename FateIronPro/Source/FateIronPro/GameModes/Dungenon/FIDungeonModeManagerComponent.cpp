...
#include "FIExperienceActionSet.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(FIDungeonModeManagerComponent)
...
UFIDungeonModeManagerComponent::UFIDungeonModeManagerComponent(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	SetIsReplicatedByDefault(true);
}

void UFIDungeonModeManagerComponent::SetCurrentExperience(FPrimaryAssetId ExperienceId)
{
	UFIAssetManager& AssetManager = UFIAssetManager::Get();
	FSoftObjectPath AssetPath = AssetManager.GetPrimaryAssetPath(ExperienceId);
	TSubclassOf<UFIExperienceDefinition> AssetClass = Cast<UClass>(AssetPath.TryLoad());
	check(AssetClass);
	const UFIExperienceDefinition* Experience = GetDefault<UFIExperienceDefinition>(AssetClass);
...
void UFIDungeonModeManagerComponent::CallOrRegister_OnExperienceLoaded_HighPriority(FOnLyraExperienceLoaded::FDelegate&& Delegate)
{
...
void UFIDungeonModeManagerComponent::CallOrRegister_OnExperienceLoaded(FOnLyraExperienceLoaded::FDelegate&& Delegate)
{
	if (IsExperienceLoaded())
	{
		Delegate.Execute(CurrentExperience);
...
void UFIDungeonModeManagerComponent::CallOrRegister_OnExperienceLoaded_LowPriority(FOnLyraExperienceLoaded::FDelegate&& Delegate)
{
	if (IsExperienceLoaded())
	{
		Delegate.Execute(CurrentExperience);
...
const UFIExperienceDefinition* UFIDungeonModeManagerComponent::GetCurrentExperienceChecked() const
{
...
bool UFIDungeonModeManagerComponent::IsExperienceLoaded() const
{
	return (LoadState == ELyraExperienceLoadState::Loaded) && (CurrentExperience != nullptr);
}

void UFIDungeonModeManagerComponent::OnRep_CurrentExperience()
{
	StartExperienceLoad();
}

void UFIDungeonModeManagerComponent::StartExperienceLoad()
{
	check(CurrentExperience != nullptr);
	check(LoadState == ELyraExperienceLoadState::Unloaded);

	UE_LOG(LogFIExperience, Log, TEXT("EXPERIENCE: StartExperienceLoad(CurrentExperience = %s, %s)"),
	...
	for (const TObjectPtr<UFIExperienceActionSet>& ActionSet : CurrentExperience->ActionSets)
	{
...
void UFIDungeonModeManagerComponent::OnExperienceLoadComplete()
{
	check(LoadState == ELyraExperienceLoadState::Loading);
	check(CurrentExperience != nullptr);

	UE_LOG(LogFIExperience, Log, TEXT("EXPERIENCE: OnExperienceLoadComplete(CurrentExperience = %s, %s)"),
		*CurrentExperience->GetPrimaryAssetId().ToString(),
		*GetClientServerContextString(this));
	...
	if (NumGameFeaturePluginsLoading > 0)
	{
		...
		{
			UFIDungeonModeManager::NotifyOfPluginActivation(PluginURL);
...
void UFIDungeonModeManagerComponent::OnGameFeaturePluginLoadComplete(const UE::GameFeatures::FResult& Result)
{
...
void UFIDungeonModeManagerComponent::OnExperienceFullLoadCompleted()
{
	check(LoadState != ELyraExperienceLoadState::Loaded);
...
void UFIDungeonModeManagerComponent::OnActionDeactivationCompleted()
{
...
void UFIDungeonModeManagerComponent::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
...
void UFIDungeonModeManagerComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	...
	for (const FString& PluginURL : GameFeaturePluginURLs)
	{
		if (UFIDungeonModeManager::RequestToDeactivatePlugin(PluginURL))
		{
	...
	if (LoadState == ELyraExperienceLoadState::Loaded)
	{
		...
		{
			UE_LOG(LogFIExperience, Error, TEXT("Actions that have asynchronous deactivation aren't fully supported yet in Lyra experiences"));
		}
...
bool UFIDungeonModeManagerComponent::ShouldShowLoadingScreen(FString& OutReason) const
{
	if (LoadState != ELyraExperienceLoadState::Loaded)
	{
...
void UFIDungeonModeManagerComponent::OnAllActionsDeactivated()
{
